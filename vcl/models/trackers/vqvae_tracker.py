...
class Vqvae_Tracker_V17(Vqvae_Tracker_V16):
    ...
    def __init__(self, target_ce_loss=False, mask_radius=-1, vq_size=32, *args, **kwargs):
        super().__init__(mask_radius=mask_radius, vq_size=32, *args, **kwargs)
        self.target_ce_loss = target_ce_loss
        
        if mask_radius != -1:
            self.mask1 = make_mask(vq_size * 2, mask_radius * 2)
        else:
            self.mask = None
    ...
    def forward_train(self, imgs, mask_query_idx, images_lab=None):
        ...
        if self.patch_size != -1:
            ...
            _, att1 = non_local_attention(tar1, refs1, per_ref=self.per_ref, temprature=self.temperature, mask=self.mask1, scaling=self.scaling_att)
            _, att2 = non_local_attention(tar2, refs2, per_ref=self.per_ref, temprature=self.temperature, mask=self.mask, scaling=self.scaling_att)
