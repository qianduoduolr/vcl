...
def coords_grid(batch: int, xx: Tensor, yy: Tensor):
    """Coordinate grid.
    Args:
...
class CorrLookup_Infer(nn.Module):
    ...
    def forward(self, corr_pyramid: Sequence[Tensor], grid=None, mode='bilinear', align_corner=True, padding_mode='zeros', shape=(32,32), mask_oob=False):
        ...
        # for window
        dx = torch.linspace(
            -self.r, self.r, 2 * self.r + 1, device=corr_pyramid[0].device)
        dy = torch.linspace(
            -self.r, self.r, 2 * self.r + 1, device=corr_pyramid[0].device)
        ...
        for i, corr in enumerate(corr_pyramid):
            ...
            centroid_lvl = grid.reshape(B * S, 1, 1, 2) / 2**i
            coords_lvl = centroid_lvl + delta_lvl

            if mask_oob:
                mask = (0 <= coords_lvl[:,:,:,0]) * \
                        (coords_lvl[:,:,:,0] <= shape[0]-1) * \
                        (0 <= coords_lvl[:,:,:,1]) * \
                        (coords_lvl[:,:,:,1] <= shape[1]-1)
            
            corr = bilinear_sample(corr, coords_lvl, mode,
                                padding_mode, align_corner)
            corr = corr.view(B, S, -1)

            if mask_oob:
                corr.masked_fill_(~mask.reshape(B, S, -1).bool(), float('-inf'))

            out_corr_pyramid.append(corr)
          
       
        out = torch.cat(out_corr_pyramid, dim=-1)
        return out.permute(0, 2, 1).contiguous().float()
...
class CorrLookup(nn.Module):
    ...
    def forward(self, corr_pyramid: Sequence[Tensor], flow: Tensor):
        """Forward function of Correlation lookup.
        Args:
            corr_pyramid (Sequence[Tensor]): Correlation pyramid.
            flow (Tensor): Current estimated optical flow.
        ...
        # for window
        dx = torch.linspace(
            -self.r, self.r, 2 * (self.r//self.interval) + 1, device=corr_pyramid[0].device)
        dy = torch.linspace(
            -self.r, self.r, 2 * (self.r//self.interval) + 1, device=corr_pyramid[0].device)
        delta = torch.stack(torch.meshgrid(dy, dx), axis=-1)
        delta_lvl = delta.view(1, 2 * self.r + 1, 2 * self.r + 1, 2)
...
@OPERATORS.register_module()
class CorrLookupSample(nn.Module):
    """Correlation lookup operator.
    This operator is used in `RAFT<https://arxiv.org/pdf/2003.12039.pdf>`_
    Args:
        radius (int): the radius of the local neighborhood of the pixels.
    ...
    def forward(self, corr_pyramid: Sequence[Tensor], flow: Tensor):
        ...
        flow = flow[:,:,::self.interval, ::self.interval]
        xx = torch.arange(0, W, device=flow.device)[::self.interval]
        yy = torch.arange(0, H, device=flow.device)[::self.interval]
        grid = coords_grid(B, xx, yy) + flow  # shape N, 2, H/s, W/s
        grid = grid.permute(0, 2, 3, 1)  # shape N, H/s, W/s, 2
        ...
        dy = torch.linspace(
            -self.r, self.r, 2 * (self.r//self.interval) + 1, device=corr_pyramid[0].device)
        delta = torch.stack(torch.meshgrid(dy, dx), axis=-1)
        delta_lvl = delta.view(1, 2 * self.r//self.interval + 1, 2 * self.r//self.interval + 1, 2)

        out_corr_pyramid = []
        for i, corr in enumerate(corr_pyramid):

            centroid_lvl = grid.reshape(B * (H//self.interval) * (W//self.interval), 1, 1, 2) / 2**i
            coords_lvl = centroid_lvl + delta_lvl

            corr = bilinear_sample(corr, coords_lvl, self.mode,
                                self.padding_mode, self.align_corners)
            corr = corr.view(B, (H//self.interval), (W//self.interval), -1)
            out_corr_pyramid.append(corr)
