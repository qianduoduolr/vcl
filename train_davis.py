...
def get_arguments():
	...
	parser.add_argument("--base-learning-rate", type=float, help="batch size",default=1e-5)
	...
	parser.add_argument("--eval-freq", type=int, help="evaluate per x iters",default=10000000)
	parser.add_argument("--print-freq", type=int, help="log per x iters",default=100)
	parser.add_argument("--save-freq", type=int, help="log per x iters",default=100000)

	parser.add_argument("--davis-freq", type=int, help="log per x iters",default=1300000000)
...
def main(args, logger):
	...
	# build model
	model = STM_SCL(args.backbone).cuda()
	...
	optimizer = torch.optim.Adam(params,lr = args.base_learning_rate, eps=1e-8, betas=[0.9,0.999])
	...
	for iter_ in range(args.total_iter):
		...
		if data_flag == 'youtube':
			n3_logit, q_feat = model(Fs[:,:,2], n12_keys, n12_values, torch.tensor([num_objects]).cuda(), True)
		...
		if data_flag == 'youtube':
			...
			if num_pixel == 0:
				...
				if num_pixel3 == 1:
					...
					q_feat = (q_feat * q_mask).reshape(1, feat_dim, -1).sum(-1)
					q_region = q_feat
			...
			loss = n2_loss + n3_loss + loss_cts + loss_cts_region
		...
		if args.multi:
			if dist.get_rank() == 0:
				if data_flag == 'youtube':
					...
					summary_writer.add_scalar('Train/loss_n2', n2_loss.item(), iter_ + 1)
		...
		if (iter_+1) % args.print_freq == 0:
			...
			if data_flag == 'youtube':
				logger.info('Train: [{:>3d}]/[{:>4d}] BT={:>0.3f} / {:>0.3f} Loss={:>0.3f} {:>0.3f} {:>0.3f} {:>0.3f} {:>0.3f} / {:>0.3f} {:>0.3f} '.format(iter_+1, args.total_iter, batch_time, time_meter.avg, loss.item(), loss_cts.item(), loss_cts_region.item(), n2_loss.item(), n3_loss.item(), loss_meter.avg, loss_meter_cts.avg))
				loss_meter.reset()
		...
		if (iter_+1) % args.eval_freq == 0:
			if args.multi:
				if dist.get_rank() == 0:
					...
					g_res, vid_jfm = evaluate(model,Testloader,['J','F'])
...
if __name__ == '__main__':
	...
	if opt.multi:
		...
		exp_path = 'log-sclstm-{args.backbone}-iters{args.total_iter}-nce-t{args.nce_t}-nce-k{args.nce_k}-expname{args.expname}'.format(args=opt)
		...
		logger = setup_logger(output=logs_dir, distributed_rank=distributed_rank, name="sclstm")
		...
		logger = setup_logger(output=None, distributed_rank=0, name="sclstm")
