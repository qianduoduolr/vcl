...
exp_name = 'temp_res18_d2_l2_rec_ssm'
docker_name = 'bit:5000/lirui_torch1.8_cuda11.1_corres'

# model settings
model = dict(
    type='Memory_Tracker_Custom_Mo',
    backbone=dict(type='ResNet',depth=18, strides=(1, 2, 1, 4), out_indices=(2,), pool_type='none'),
    loss_weight=dict(l1_loss=1, sm_loss=1),
    sm_loss=dict(type='SmoothnessLoss'),
    cvt_grid=True,
    downsample_rate=[4,],
    radius=[12,],
    temperature=1,
...
model_test = dict(
    type='VanillaTracker',
    backbone=dict(type='ResNet',depth=18, strides=(1, 2, 1, 1), out_indices=(2, ), pool_type='none'),
)
...
visual_config = dict(type='VisualizationHook', interval=100, res_name_list=['corr', 'imgs'], output_dir=work_dir+'/vis')
