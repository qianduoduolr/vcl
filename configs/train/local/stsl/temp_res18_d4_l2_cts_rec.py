...
exp_name = 'temp_res18_d4_l2_cts_rec'
docker_name = 'bit:5000/lirui_torch1.8_cuda11.1_corres'

# model settings
# model settings
model = dict(
    type='Memory_Tracker_Custom_MoCo',
    backbone=dict(type='ResNet',depth=18, strides=(1, 2, 2, 1), out_indices=(3,), pool_type='none', dilations=(1,1,2,4)),
    backbone_m=dict(type='ResNet',depth=18, strides=(1, 2, 2, 1), out_indices=(3,), pool_type='none', dilations=(1,1,2,4)),
    loss_weight=dict(cts_rec_loss=1),
    num_k=32,
    downsample_rate=[8,],
    radius=[6,],
    temperature=1,
...
optimizers = dict(
    backbone=dict(
    type='SGD', lr=0.01, weight_decay=0.0001, momentum=0.9
    )
)
# learning policy
# total_iters = 200000
