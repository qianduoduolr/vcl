...
exp_name = 'train_vqvae_video_d4_nemd32_contrastive_vfs'
docker_name = 'bit:5000/lirui_torch1.5_cuda10.1_corres'

# model settings
model = dict(
    type='VQCL',
    backbone=dict(type='ResNet', depth=50, strides=(1, 2, 1, 1), out_indices=(3, )),
    K=16384,
    m=0.999,
    T=0.1,
    embed_dim=128,
    ...
    pretrained='/home/lr/models/ssl/vcl/vfs_pretrain/r50_nc_sgd_cos_100e_r5_1xNx2_k400-d7ce3ad0.pth'
)
...
train_pipeline = [
    ...
    dict(type='Collect', keys=['imgs', 'bboxs'], meta_keys=[]),
    dict(type='ToTensor', keys=['imgs', 'bboxs'])
]
...
checkpoint_config = dict(interval=2, save_optimizer=True, by_epoch=True)
