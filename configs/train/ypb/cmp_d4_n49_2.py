...
exp_name = 'cmp_d4_n49_2'
docker_name = 'bit:5000/lirui_torch1.8_cuda11.1_corres'

# model settings
model = dict(
    type='CMP',
    backbone=dict(type='ResNet',depth=18, strides=(1, 2, 2, 1), out_indices=(3, ), dilations=(1, 1, 2, 4), pool_type='none'),
    loss=dict(type='DiscreteLoss', nbins=49, fmax=50),
    output_dim=98
)

model_test = dict(
    type='VanillaTracker',
    backbone=dict(type='ResNet',depth=18, strides=(1, 2, 2, 1), out_indices=(2, ), dilations=(1, 1, 2, 4), pool_type='none'),
)
...
train_pipeline = [
    ...
    dict(type='Resize', scale=(256, 256), keep_ratio=False, keys='flows'),
    dict(type='Flip', flip_ratio=0.5),
    dict(type='Flow_Sampler'),
    dict(type='Normalize', **img_norm_cfg),
    dict(type='FormatShape', input_format='NCTHW'),
    dict(type='FormatShape', input_format='NCTHW', keys='flows'),
    dict(type='Collect', keys=['imgs', 'flows', 'sparse', 'mask'], meta_keys=[]),
    dict(type='ToTensor', keys=['imgs', 'flows', 'sparse', 'mask'])
]
...
data = dict(
    ...
    train=
            ...
            data_prefix=dict(RGB='train/JPEGImages_s256', FLOW='Flows_s256'),
            clip_length=2,
