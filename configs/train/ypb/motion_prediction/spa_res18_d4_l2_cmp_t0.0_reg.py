...
exp_name = 'spa_res18_d4_l2_cmp_t0.0_reg'
...
model = dict(
    ...
    downsample_rate=[4,8],
    feat_size=[64,32],
    cmp_loss=dict(type='Soft_Ce_Loss'),
    mode='regression',
    loss_weight=dict(l1_loss=0, dist_loss=0, cmp_loss=1),
    detach=True
)
...
log_config = dict(
    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook', init_kwargs=dict(project='video_correspondence_cmp', name=exp_name, config=model), log_artifact=False),
    ])
