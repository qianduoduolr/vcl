...
exp_name = 'sft_d4_l2_2'
docker_name = 'bit:5000/lirui_torch1.8_cuda11.1_corr'

# model settings
model = dict(
    type='Single_Frame_Tracker',
    backbone=dict(type='ResNet',depth=18, strides=(1, 2, 2, 1), out_indices=(2, ), pool_type='none'),
    backbone_t=dict(type='ResNet',depth=50, strides=(1, 2, 1, 1), out_indices=(3, ),pretrained='/gdata/lirui/models/ssl/image_based/detco_200ep_AA.pth'),
    downsample_rate=8,
    upsample=False
)
...
model_test = dict(
    type='VanillaTracker',
    backbone=dict(type='ResNet',depth=18, strides=(1, 2, 2, 1), out_indices=(2, ), pool_type='mean'),
)
...
optimizers = dict(
    ...
    flow_conv=dict(type='Adam', lr=0.001, betas=(0.9, 0.999)),
    )
# learning policy
# total_iters = 200000
runner_type='epoch'
