...
exp_name = 'mast_d4_l2_pyramid_inter_9'
...
model = dict(
    ...
    backbone_m=dict(type='ResNet',depth=18, strides=(1, 2, 2, 1), out_indices=(2, ), pool_type='none'),
    loss=dict(type='MSELoss',reduction='mean'),
    K=8192,
    pretrained='/gdata/lirui/expdir/VCL/group_vqvae_tracker/mast_d4_l2_pyramid_dis_11/epoch_3200.pth',
    loss_weight = dict(stage0_l1_loss=0, stage1_l1_loss=1, layer_dist_loss=1000),
...
evaluation = dict(output_dir=f'{work_dir}/eval_output_val', interval=50, by_epoch=True
                  )
