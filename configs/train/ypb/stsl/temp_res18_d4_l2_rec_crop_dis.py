...
exp_name = 'temp_res18_d4_l2_rec_crop_dis'
docker_name = 'bit:5000/lirui_torch1.8_cuda11.1_corres'

# model settings
# model settings
model = dict(
    type='Memory_Tracker_Custom_Crop',
    backbone=dict(type='ResNet',depth=18, strides=(1, 2, 2, 4), out_indices=(2,), pool_type='none'),
    backbone_a=dict(type='ResNet',depth=18, strides=(1, 2, 2, 4), out_indices=(2,), pool_type='none'),
    loss_weight=dict(main_l1_loss=1, crop_l1_loss=1, reg_loss=1000),
    downsample_rate=[8,],
    radius=[6,],
    temperature=1,
    feat_size=[32,],
...
train_pipeline = [
    ...
    dict(type='FrameDup', keys_list=['imgs','imgs'], out_keys_list=['imgs', 'imgs_crop']),
    dict(type='RandomScaleCrop', keys='imgs_crop', center_crop=True, crop_size=64),
    # dict(type='Flip', flip_ratio=0.5),
    dict(type='RGB2LAB', keys='imgs', output_keys='images_lab'),
    dict(type='RGB2LAB', keys='imgs_crop', output_keys='images_lab_crop'),

    dict(type='Normalize', **img_norm_cfg_lab, keys='images_lab'),
    dict(type='Normalize', **img_norm_cfg_lab, keys='images_lab_crop'),

    dict(type='GetAffanity', keys='imgs_crop', size=(256, 256)),

    dict(type='FormatShape', input_format='NPTCHW', keys='images_lab'),
    dict(type='FormatShape', input_format='NPTCHW', keys='images_lab_crop'),

    dict(type='Collect', keys=['images_lab', 'images_lab_crop', 'affine_imgs_crop'], meta_keys=[]),
    dict(type='ToTensor', keys=['images_lab_crop', 'images_lab', 'affine_imgs_crop'])
]
