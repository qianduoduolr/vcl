...
exp_name = 'spa_temp_res50_d2_l2_rec_local_dist_t0.7_12'
docker_name = 'bit:5000/lirui_torch1.8_cuda11.1_corres'

# model settings
model = dict(
    type='Framework_V2',
    backbone=dict(type='ResNet',depth=50, strides=(1, 1, 1, 4), out_indices=(2, 3), pool_type='mean', pretrained='/gdata/lirui/models/ssl/image_based/detco_200ep_AA.pth'),
    ...
    momentum=-1,
    detach=True,
    loss_weight = dict(stage0_l1_loss=1, stage1_l1_loss=0, layer_dist_loss=0, correlation_dist_loss=200),
    pretrained=None
)

model_test = dict(
    type='VanillaTracker',
    backbone=dict(type='ResNet',depth=50, strides=(1, 1, 1, 4), out_indices=(2, ), pool_type='mean'),
    head=dict(type='MlpHead', head_in=1024, head_out=256, use_norm=True),
)
...
optimizers = dict(
    ...
    head=dict(type='Adam', lr=0.00005, betas=(0.9, 0.999))
    )
# learning policy
# total_iters = 200000
runner_type='epoch'
...
evaluation = dict(output_dir=f'{work_dir}/eval_output_val', interval=200, by_epoch=True
                  )
