...
def create_lmdb_video_dataset_rgb(root_path, dst_path, workers=-1, quality=100, resize=True, save_jpg=False):

    videos = glob(os.path.join(root_path,'*'))
    print('begin')

    def make_video(video_path, dst_path=None, resize=True, save_jpg=False):
        dst_file = video_path.replace('JPEGImages', dst_path)
        dst_file_jpg = video_path.replace('JPEGImages', 'JPEGImages_s256')
