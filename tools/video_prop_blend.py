...
def blend_for_vip():
    data_root = '/home/lr/dataset/VIP/VIP_Fine/Images'
    input_dir = '/home/lr/expdir/VCL/group_vqvae_tracker/final_framework_v2_11/human_part_eval_outputindices2'
    video_list = glob.glob(osp.join(input_dir, '*/'))
    for v in video_list:
        results = sorted(glob.glob(osp.join(v, '*.png')))
        ...
        for idx, r in enumerate(results):
            ...
            output_path = r.replace('human_part_eval_outputindices2', 'human_part_eval_outputindices2_blend')
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
            out.save(output_path)
            
def blend_for_vos():
    data_root = '/home/lr/dataset/DAVIS/JPEGImages/480p'
    input_dir = '/home/lr/expdir/VCL/group_vqvae_tracker/vfs_d4_res50_eval_2/eval_outputindices2'
    video_list = glob.glob(osp.join(input_dir, '*/'))
